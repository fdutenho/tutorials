   ...
   constructor(props) {
      ...
      subscription = postal.subscribe({
         ...
         callback: function(data, envelope) {
            console.info("floor: received message that an elevator reached floor " + JSON.stringify(data));

            if(this.state.floorButtons[(data.floor+floorAdjust)].state === true) {
               console.info("floors: remove signal for floor " + data.floor);
               var tempFloorButtons = this.state.floorButtons;
               console.debug("tempState: " + JSON.stringify(tempFloorButtons));
               tempFloorButtons[(data.floor+floorAdjust)].state = false;
               this.setState({floorButtons: tempFloorButtons});

               console.debug("state: " + JSON.stringify(this.state));
               this.refs["btnFloor"+data.floor].classList.toggle('btn-outline-secondary');
               this.refs["btnFloor"+data.floor].classList.toggle('btn-secondary');
            } else {
               console.debug("floors: floor "+ data.floor + " has no signal");
            }
           }.bind(this)
        });
   }
